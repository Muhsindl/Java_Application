...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Maksimum notları bulma
        int[] maxNot = new int[col]; // Ders sayısına göre boyutlandırılan dizi

        // Başlangıçta her dersin ilk notunu maksimum olarak kabul ediyoruz
        for (int j = 0; j < col; j++) {
            maxNot[j] = matris[0][j];
        }

        // Her dersin maksimum notunu bulma
        for (int j = 0; j < col; j++) {
            for (int i = 1; i < row; i++) {
                if (matris[i][j] > maxNot[j]) {
                    maxNot[j] = matris[i][j];
                }
            }
        }

        // Her ders için maksimum notları ekrana yazdırma
        System.out.println("Her ders için maksimum notlar:");
        System.out.println(Arrays.toString(maxNot));
    }
}
